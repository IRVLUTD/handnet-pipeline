...
from dex_ycb_toolkit.log import get_logger
...
from freihand.eval import align_w_scale, curve, createHTML
...
class HPEEvaluator():
  ...
  def __init__(self, name):
    ...
    # 'ab' - absolute, 'rr' route relative, 'pa'- procustes
    self._epoch_metrics = {
        'ab': {},
        'rr': {},
        'pa': {}
  ...
  def _load_results(self, res_file, root3d=False):
    ...
    with open(res_file, 'r') as f:
      for line in f:
        ...
        if root3d:
          epoch = os.path.basename(res_file)[8:-4]
          root3d_file = os.path.join(os.path.dirname(res_file), f'root3d_{epoch}/{str(image_id)}.txt')
          try:
            with open(root3d_file, 'r') as g:
              for line in g:
                elems = line.split(',')
                root_3d = np.array(elems[:], dtype=np.float64).reshape(3) * 1000
                diff = root_3d - joint_3d[0]
                results[image_id] = joint_3d + diff
          except:
            pass
    return results

  def return_joints(self, res_file, out_dir=None):
    if out_dir is None:
      out_dir = self._out_dir

    res_name = os.path.splitext(os.path.basename(res_file))[0]
    log_file = os.path.join(out_dir,
                            "hpe_eval_{}.log".format(res_name))
  ...
  def align_with_scale(self, kpt_gt, kpt_pred):
    return align_w_scale(kpt_gt, kpt_pred)

  def evaluate(self, epoch, res_file, out_dir=None, root3d=False):
    ...
    res = self._load_results(res_file, root3d=root3d)
    ...
    self._epoch_metrics['ab'][f'{epoch}'] = (mean_ab, auc_ab, pck_ab, thresh_ab)
    self._epoch_metrics['rr'][f'{epoch}'] = (mean_rr, auc_rr, pck_rr, thresh_rr)
    self._epoch_metrics['pa'][f'{epoch}'] = (mean_pa, auc_pa, pck_pa, thresh_pa)
