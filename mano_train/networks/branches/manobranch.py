...
class ManoBranch(nn.Module):
    def __init__(
        ...
        self.mano_pose_size = mano_pose_size
        if not self.use_pca:
    ...
    def forward(
        ...
        verts = inp.new_empty((inp.shape[0], 778, 3)).float()
        joints = inp.new_empty((inp.shape[0], 21, 3)).float()
        pose = inp.new_empty((inp.shape[0], self.mano_pose_size)).float()
        trans = inp.new_empty((inp.shape[0], 3))
        if pose_right.shape[0] > 0:
            verts[is_rights] = verts_right
            joints[is_rights] = joints_right
            pose[is_rights] = pose_right
            if self.use_trans:
                trans[is_rights] = trans_right
        if pose_left.shape[0] > 0:
            verts[is_lefts] = verts_left
            joints[is_lefts] = joints_left
            pose[is_lefts] = pose_left
            if self.use_trans:
                trans[is_lefts] = trans_left
        if shape is not None:
            shape = inp.new_empty((inp.shape[0], 10)).float()
            if shape_right.shape[0] > 0:
                shape[is_rights] = shape_right
            if shape_left.shape[0] > 0:
    ...
    def compute_loss(self, preds, target):
        ...
        if self.lambda_pose_reg:
            pose_reg_loss = torch_f.mse_loss(
                preds['pose'], target['pose']
            )
            final_loss += self.lambda_pose_reg * pose_reg_loss
            mano_losses["pose_reg"] = pose_reg_loss
        
        if self.lambda_trans:
            trans_loss = torch_f.mse_loss(
                preds['trans'], target['trans']
            )
            final_loss += self.lambda_trans * trans_loss
            mano_losses["trans"] = trans_loss
