...
def get_e2e_loaders(args, detect=False, a2j=False, distributed=True):
    if detect:
    ...
    if os.path.exists(f'{cache_dir}train.pkl'):
        ...
        train_sampler = torch.utils.data.DistributedSampler(dataset) if a2j and distributed else torch.utils.data.RandomSampler(dataset)
    ...
    if os.path.exists(f'{cache_dir}test.pkl'):
        ...
        test_sampler = torch.utils.data.DistributedSampler(dataset, shuffle=False) if a2j and distributed else torch.utils.data.SequentialSampler(dataset)
    ...
    if os.path.exists(f'{cache_dir}val.pkl'):
        ...
        val_sampler = torch.utils.data.DistributedSampler(dataset, shuffle=False) if a2j and distributed else torch.utils.data.SequentialSampler(dataset)
