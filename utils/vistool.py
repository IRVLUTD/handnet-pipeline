...
class VisualUtil:
    ...
    def __init__(self, dataset):
        ...
        # RED BGR
        self.color_gt = [(0,0,102), (0,0,179), (0,0,255), (77,77,255), (153,153,255)]
        # self.color_pred = ['#660000', '#b30000', '#ff0000', '#ff4d4d', '#ff9999']
        # BLUE BGR
        self.color_pred = [(102,0,0), (179,0,0), (255,0,0), (255,77,77), (255,153,153)]
        # self.color_gt = ['#000066', '#0000b3', '#0000ff', '#4d4dff', '#9999ff']

    def orig_depth(self, img):
        image = img.copy()
        image = (image.squeeze() + 1) * 100
        image = image[:, :, np.newaxis].repeat(3, axis=-1)
        return image

    def plot(self, img, path, jt_uvd_gt, jt_uvd_pred=None, return_image=False):
        image = img.copy()
        
        if isinstance(jt_uvd_pred, np.ndarray):
        ...
        return image if return_image else cv2.imwrite(path, image)
    ...
    def _get_setting(self):
        ...
        elif self.dataset == 'msra' or self.dataset=='dexycb':
