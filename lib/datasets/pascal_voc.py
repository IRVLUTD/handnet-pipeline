...
class pascal_voc(imdb):
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            hs = -1 if self._is_not_legimate(hs) else int(hs.text)
            contactstate[ix] = hs

            contactr = obj.find('contactright')
            contactr = -1 if self._is_not_legimate(contactr) else int(contactr.text)
            contactright[ix] = contactr

            contactl = obj.find('contactleft')
            contactl = -1 if self._is_not_legimate(contactl) else int(contactl.text)
            ...
            lr = -1 if self._is_not_legimate(lr) else float(lr.text)
            handside[ix] = lr
