...
class ImageListener:
    ...
    def run_network(self):
        ...
        joints3d = convert_joints(keypoint_pred, None, detection, parameters, 176, 176)[:, :2]
        orig_height, orig_width = im_color.shape[:2]
        out = predict_mesh(joints2d, crop_size=self.virtual_crop_size)

        out['mesh'] = out['mesh'] * 1000. + joints3d
        out['mesh'] /= 1000.
        out['mesh'][:, 1] *= -1
        out['mesh'][:, 2] *= -1
        ...
        rendered_img = render(out, orig_height, orig_width, im_color, self.mesh_model.face, color)  # s[idx])
        ...
        tmpimg = im_color.copy().astype(np.uint8)
        pose_vis_img = vis_2d_keypoints(tmpimg, tmpkps, self.skeleton)
        #cv2.imwrite(output_path + f'demo_pose2d.png', pose_vis_img)

        #save_obj(out['mesh'], self.mesh_model.face, output_path + f'demo_mesh.obj')
