...
def transform_point(trans, pt):
    """
    Transform the given point from one frame to another according to trans
    :param trans: the tf.transformation relati
                                                on between 2 frames
    :param pt: a Point object of [x,y,z] to be transformed
    :return:
    """
...
class ImageListener:
    ...
    def __init__(self, network, RGBD=False, left=False, save_path=None):
        ...
        self.hand_pc_cam_pub = rospy.Publisher('mesh_pc_cam', numpy_msg(Floats), queue_size=10)
        self.hand_pc_base_pub = rospy.Publisher('mesh_pc_base', numpy_msg(Floats), queue_size=10)
    ...
    def run_network(self,tf_buffer):
        ...
        joints3d = convert_joints(keypoint_pred, None, detection, self.paras, 176, 176, is_left=self.left, image_width=full_image.shape[1], image_height=full_image.shape[0])
        orig_height, orig_width = full_image.shape[:2]
        out = predict_mesh(self.model, joints2d, self.graph_perm_reverse, self.mesh_model)

        if self.left:
            hand_root = out['mesh'][0,:]
            out['mesh'][:, :] -= hand_root
            out['mesh'][:, 0] *= -1
            out['mesh'][:, 1] *= -1
            out['mesh'][:, 2] *= -1
        ...
        mat = quaternion_matrix(quat)
        t = np.array([transform.transform.translation.x, transform.transform.translation.y, transform.transform.translation.z]).reshape((3, 1))
        new = []
        for index in range(0, len(out['mesh'])):
            xyz = out['mesh'][index]
            point = Point()
            point.x = xyz[0]
            point.y = xyz[1]
            point.z = xyz[2]
            new_point_obj = transform_point(transform, point)
            new_point = [new_point_obj.x, new_point_obj.y, new_point_obj.z]
            new.append(new_point)
        new_numpy_array = np.array(new, dtype=np.float64)
        print(len(new), type(new))
        print(new[:3])
        print(new_numpy_array[:3])
        print(type(new_numpy_array), new_numpy_array.shape)
        print(new_numpy_array.flatten()[:9])
        RT_camera = mat.copy()
        RT_camera[:3, 3] = t.flatten()

        if args.save_path:
            filename = 0
            # find avail filename
            object_name = "006_mustard_bottle"
            while os.path.exists(os.path.join(self.save_path, f'{filename}_{object_name}.npy')):
                filename += 1
            np.save(os.path.join(self.save_path, f'{filename}_{object_name}.npy'), out['mesh'])
            np.save(os.path.join(self.save_path, f'{filename}_{object_name}_base.npy'), new)
            np.save(os.path.join(self.save_path, f'{filename}_{object_name}_cam_intrinsics.npy'), self.paras)
            np.save(os.path.join(self.save_path, f'{filename}_{object_name}_RTcamera.npy'), RT_camera)
            np.save(os.path.join(self.save_path, f'{filename}_{object_name}_keypoints.npy'), keypoint_pred)
            np.save(os.path.join(self.save_path, f'{filename}_{object_name}_full_image.npy'), full_image)
	    # vis mesh (and optionally save)
        rendered_img = render(for_render, self.paras, orig_height, orig_width, full_image, self.mesh_model.face, mesh_filename=os.path.join(self.save_path, f'{filename}_{object_name}.obj' if args.save_path else None))
        label = label[:, :, (2,1,0)]
        rendered_img = rendered_img[:, :, (2,1,0)]
        cv2.imwrite(os.path.join(self.save_path, f'{filename}_{object_name}_labeled.jpg'),label)
        cv2.imwrite(os.path.join(self.save_path, f'{filename}_{object_name}.jpg'),rendered_img )
        ...
        self.hand_pc_cam_pub.publish(out['mesh'].flatten())
        # self.hand_pc_base_pub.publish(new_numpy_array.flatten())
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        listener.run_network(tf_buffer)
